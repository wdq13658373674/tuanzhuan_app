...
var wsocket ={
	...
	/*检查重新登录方法*/
	checkLogin:function(){
    	if(wsocket.connected == false && wsocket.reconnect){
            wsocket.reconnect_count--;
            console.log('重新连接中...');
            wsocket.websocket();
		}else if(!wsocket.isloged && wsocket.reconnect){
            wsocket.reconnect_count--;
            console.log('重新登录中...');
            wsocket.callback();
        }
	...
	init:function(callback,obj){
		...
		if(typeof this.token.token != 'undefined' && this.token.token!=''){
            this.websocket();
            this.callback = callback;
		}else{
            console.warn("没有登录信息,不登录即时服务");
		}
	},
	websocket:function(){
		//建立websocket连接
